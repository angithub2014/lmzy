...
function checkOldPassWord(){
	...
	if(Length(oldpassword)<6 || Length(oldpassword)>16){
	...
	$.ajax({
		...
		success:function(data){
			if(data !==null){
				if(data.errorCode=='0'){
					addRightImages("oldpasswordImage","oldpasswordTip");
					oldpassflag=1;
					return true;
				}else{
					addWrongImages('oldpassword','oldpasswordImage','oldpasswordTip','旧密码不正确！');
					oldpassflag=0;
					return false;
				}					
			}else{
...
function modifyPassWord(){
	if(!(passflag==1 && realpassflag==1&&oldpassflag==1)){		
